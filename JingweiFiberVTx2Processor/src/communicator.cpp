...
boost::scoped_ptr<ip::tcp::socket>  aq::Communicator::magnetic_valve_socket_;
boost::scoped_ptr<ip::tcp::socket> aq::Communicator::show_image_socket_;
...
bool aq::Communicator::tcp_connect(std::string _ip, int _port, std::string &_error_str)
{
	...
	socket_.reset(new asio::ip::tcp::socket(m_io));
...
bool aq::Communicator::receive_img(std::string& _error_str, boost::scoped_ptr<ip::tcp::socket> &socket_)
{
	_error_str.clear();
	//std::int64_t size = asio::read(*socket_, asio::buffer((char*)data_buffer_.data(), sizeof(VCameraMessageTrans)));
...
//*****************************************************************************************************************
int aq::Communicator::show_ctrl_message(boost::scoped_ptr<ip::tcp::socket>& socket_,int  wParam, void* lParam)
{
	std::string strMsg;
...
int aq::Communicator::show_ctrl_message(boost::scoped_ptr<ip::tcp::socket>& socket_, int  wParam, void* lParam, int length, uchar* m_buf)
{
	std::string strMsg;
	int nCtrlSignalling = (int)wParam;
	Vsee::VCameraMessageTrans* ImageMsg = (Vsee::VCameraMessageTrans*)lParam;
...
bool aq::Communicator::show_image_connect(std::string _ip, int _port, std::string& _error_str)
{
	...
	ep_ = new asio::ip::tcp::endpoint(asio::ip::address::from_string(_ip), _port);
	show_image_socket_.reset(new asio::ip::tcp::socket(m_io_image));
	asio::error_code ec;

	show_image_socket_->connect(*ep_, ec);
	if (ec)
	{
		show_image_socket_->close();
		_error_str = "The connection could have timed out.";
		return false;
	}
